...
func walk_phase() -> void:
	npc.state = "walk"
	dirction = global_position.direction_to( target.target_postion )
	npc.dirction = dirction
	npc.velocity = walk_speed * dirction
	npc.update_dirction( target.target_postion )
	npc.update_animation()
